<template>
    ...
    <div v-else>
        ...
        <div class="border p-4 rounded-lg border-solid border-gray-300 bg-gray-50 max-w-3xl mb-10" v-else>
            <div>
    ...
    <div>
        <button @click="tryAnswer" v-if="!showAnswer"
    ...
    <div v-if="showAnswer" class="text-lg">
        ...
        <button @click="nextQuestion" 
            class="text-white bg-gray-700 my-4 hover:bg-gray-900 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-4 py-2">Question suivante
        </button>
    </div>
</template>
...
const tryAnswer = async () => {
    ...
    if (drill.value[0].question.type === 'fill-in-the-gaps' || drill.value[0].question.type === 'multiple-choice') {
        data = await drillStore.tryAnswer(drill.value[0].question.id, choices.value, drill.value[0].id);
    } else {
        data = await drillStore.tryAnswer(drill.value[0].question.id, answer.value, drill.value[0].id);
    }
